...
namespace AdvantIQ.ExampleAdapters.Output.WinFormStacker
{
    ...
    /// </summary>
    public class StackerEdgeOutput<T> : TypedEdgeOutputAdapter<T>
    {
        ...
        public StackerEdgeOutput(StackerConfig config)
        {
                ...
                while (AdapterState == AdapterState.Running && !stopFlag)
                {
                    ...
                    else
                    {
                        ...
                        if (currEvent.EventKind == EventKind.Insert)
                        {
                            ...
                            sb.AppendFormat("[{0} {1}] ", currEvent.StartTime, currEvent.EdgeType.ToString());
                            var sep = "";
        ...
        private void PrepareToStop(EdgeEvent<T> currEvent, DequeueOperationResult result)
        {
