...
namespace AdvantIQ.ExampleAdapters.Output.WinFormStacker
{
    ...
    /// </summary>
    public class StackerPointOutput<T> : TypedPointOutputAdapter<T>
    {
        ...
        public StackerPointOutput(StackerConfig config)
        {
                ...
                while (AdapterState == AdapterState.Running && !stopFlag)
                {
                    ...
                    else
                    {
                        ...
                        if (currEvent.EventKind == EventKind.Insert)
                        {
                            ...
                            sb.AppendFormat("[{0}] ", currEvent.StartTime.ToLocalTime());
                            var sep = "";
