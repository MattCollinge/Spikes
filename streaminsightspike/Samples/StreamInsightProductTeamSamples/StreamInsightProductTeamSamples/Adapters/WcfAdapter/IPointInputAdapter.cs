...
namespace StreamInsight.Samples.Adapters.Wcf
{
    ...
    /// <summary>
    /// Service contract for the point input adapter.
    /// </summary>
    [ServiceContract]
    public interface IPointInputAdapter
    {
        /// <summary>
        /// Attempts to enqueue the given point event. The result code indicates whether the operation
        /// has succeeded, the adapter is suspended -- in which case the operation should be retried later --
        /// or whether the adapter has stopped and can no longer accept events.
        /// </summary>
        [OperationContract]
